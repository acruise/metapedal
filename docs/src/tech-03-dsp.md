...
The DSP capacity is consumed by two broad categories of processing that compete for the same H7 cycles: audio effects and synthesis (the traditional pedalboard purposes), and audio analysis (extracting control signals from incoming audio). Effects and synthesis run at audio sample rate and dominate when they are present. Audio analysis algorithms vary widely in cost: envelope followers and transient detectors are nearly free; pitch detectors and beat trackers are moderate; sophisticated spectral analyses cost more. The DSP budget analysis later in this document covers effects and synthesis in detail. Audio analysis algorithms add to the budget when configured, and modules dedicated primarily to analysis (rather than effects) have substantially more headroom for analysis-heavy configurations. The TODO document captures the planned algorithm catalog and the v1 versus v1.x scoping for audio analysis features.
